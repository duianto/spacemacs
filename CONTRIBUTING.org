...
**** README.org tags
A layers README.org file should have a =#+TAGS:= line:
...
Individual tags are separated by a "|" (pipe, vertical bar) character.

The example above has 2 tags: "layer" and "web service"

The valid tags are listed in: [[https://github.com/syl20bnr/spacemacs/blob/develop/.ci/spacedoc-cfg.edn][<spacemacs_root>/.ci/spacedoc-cfg.edn]]
...
Labels are used to name the headlines in [[https://github.com/syl20bnr/spacemacs/blob/develop/layers/LAYERS.org][LAYERS.org]].

The tags structure is defined in =:spacetools.spacedoc.config/layers-org-query=
by chaining the tags into a tree. Every leaf in the tree turns into a list of
descriptions that are pulled from the =README.org= files that match a leaf's
path in the tree.

This makes sure that every layer description only appears once.

You can see how the shape of [[https://github.com/syl20bnr/spacemacs/blob/develop/.ci/spacedoc-cfg.edn][<spacemacs_root>/.ci/spacedoc-cfg.edn]] repeats in
[[https://github.com/syl20bnr/spacemacs/blob/develop/layers/LAYERS.org#table-of-contents][LAYERS.org]]'s table of contents.

?
It is used for "catch all" sections to match layers which do not have a precise
grouping.

For example in the =DSL= language layer:
...
Here every layer that has the tags: "layer", "programming", "dsl", plus one of:
"lisp", "markup" or "script", will be displayed before the rest of the layers
that only have the tags: "layer", "programming", "dsl".
